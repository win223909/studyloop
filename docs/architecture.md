...
1. A course plan is created from a topic plus retrieved or supplied evidence. Automatic search first makes one `learning_request` call to organize the subject, goal, and at most three initial queries without replacing the original topic. This goal is a retrieval hint, not evidence. The call uses the [default prompt](../server/core/learning-request.js), at most 4,096 output tokens, and a 30-second timeout. Malformed or truncated output, or an invalid result shape falls back to existing keyword extraction; explicit refusals and service failures stop the request. Candidate titles, summaries, and short introductions are screened for relevance before material is sent to the model. The model checks coverage against the complete original topic and learning level. Insufficient coverage can trigger one follow-up round using up to three model-suggested concepts or synonyms, followed by a final model check. Pasted/uploaded material skips request preparation and external search. User-supplied and retrieved text, including suggested queries, are data, never instructions.
...
A normal new course uses one outline call, plus one request-preparation call for automatic search; the bounded keyword-search fallback permits a second planning call. With permitted format recovery, a search outline uses at most five model calls. The question bank uses three further calls (authoring, blind answer review, and explanation/evidence review). Each Wikipedia round uses at most twelve HTTP requests; an initial topic kept as one query uses at most four. Deduplicated evidence is capped at eight sources and 36,000 characters. If coverage remains insufficient, no plan is saved; the response can offer narrower topics and material-entry actions. The review model can differ from the authoring model while sharing the configured provider. Model reviews remain fallible. A fixed arithmetic solver, a human moderation queue, curriculum certification, and automatic repair of rejected question banks are not currently implemented.
