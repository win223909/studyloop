...
- A verified classroom completion receipt and explicit progress sync that never rewrites an original attempt.
- Stronger lesson quality checks, age-appropriate prompts, mobile usability, and accessible classroom navigation.
- Broader real-classroom acceptance coverage for narration, interaction types, failure recovery, and supported live providers. Existing local HTTP/browser fixtures do not establish paid-provider quality.
...
The project will distinguish generated content from original examples, material snippets from full sources, adapter tests from real-provider tests, and proposed integrations from shipped classroom behaviour. OpenMAIC acknowledgments and license obligations remain part of the interface, repository, and the bundled runtime.

中文说明：目前已内置 OpenMAIC 经典课堂并支持从答卷生成短课；下一阶段优先完善进度回传、短课体验、移动端与异常恢复，并提高逐题来源和可计算内容的核验能力。学生账户、跨设备恢复、家长/教师角色、间隔复习、OCR 与更多题型随后扩展。路线图中的功能不代表当前版本已经具备。
