...
- **查看依据**：保留课程来源和每题引用。关键词默认搜索 Wikipedia，从题型长句提取明确知识点，并根据候选标题、摘要和短导语预筛相关性；模型仍核对完整原主题。覆盖不足时，模型最多提出三个细分检索词，自动补查一轮并再次核对，仍不足就明确失败。可选 Brave Search 扩展网页摘要来源，并保留摘要标记。
