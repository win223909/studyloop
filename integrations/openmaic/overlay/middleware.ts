...
export async function middleware(request: NextRequest) {
  if (process.env.NEXT_PUBLIC_STUDYLOOP_EMBEDDED === 'true') {
    const expected = process.env.STUDYLOOP_RUNTIME_TOKEN;
    const received = request.headers.get('x-studyloop-runtime') || '';
    let mismatch = !expected || expected.length !== received.length ? 1 : 0;
    if (expected) {
      for (let index = 0; index < expected.length; index++) {
        mismatch |= expected.charCodeAt(index) ^ (received.charCodeAt(index) || 0);
      }
    }
    if (mismatch) return new NextResponse('Not found', { status: 404 });
  }
