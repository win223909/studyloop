...
const STUDYLOOP_EMBEDDED = process.env.NEXT_PUBLIC_STUDYLOOP_EMBEDDED === 'true';
const MAX_EMBEDDED_SCENES = 6;
const SCENE_TYPES: SceneType[] = STUDYLOOP_EMBEDDED
  ? ['slide', 'quiz', 'interactive']
  : ['slide', 'quiz', 'interactive', 'pbl'];

export function getStudyLoopOutlineError(outlines: SceneOutline[], locale: string): string | null {
  if (!STUDYLOOP_EMBEDDED) return null;
  const zh = locale.startsWith('zh');
  if (outlines.length > MAX_EMBEDDED_SCENES) {
    return zh
      ? '内置课堂最多支持 6 个场景，请删除多余场景后再确认生成。'
      : 'The built-in classroom supports up to 6 scenes. Remove extra scenes before confirming.';
  }
  if (outlines.some((outline) => !SCENE_TYPES.includes(outline.type))) {
    return zh
      ? '内置课堂支持幻灯片、测验和互动场景。请更换不支持的场景类型后再确认生成。'
      : 'The built-in classroom supports slides, quizzes and interactive scenes. Change unsupported scene types before confirming.';
  }
  return null;
}
...
export function OutlinesEditor({
  ...
  const atSceneLimit = STUDYLOOP_EMBEDDED && outlines.length >= MAX_EMBEDDED_SCENES;
  const addingDisabled = editingDisabled || atSceneLimit;
  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          ...
          {STUDYLOOP_EMBEDDED && (
            <p
              ...
              role={atSceneLimit ? 'status' : undefined}
            >
              {locale.startsWith('zh')
                ? atSceneLimit
                  ? '已达到 6 个场景上限。删除一个场景后可以继续新增。支持幻灯片、测验和互动场景。'
                  : '内置课堂支持幻灯片、测验和互动场景，最多 6 个场景。'
                : atSceneLimit
                  ? 'The 6-scene limit is reached. Remove a scene to add another. Slides, quizzes and interactive scenes are supported.'
                  : 'The built-in classroom supports up to 6 scenes: slides, quizzes and interactive scenes.'}
            </p>
          )}
        </div>
        {onCollapse && (
      ...
      <div className="relative max-h-[64vh] overflow-y-auto px-3 pb-2 md:px-6">
        {outlines.length === 0 ? (
          <EmptyState isStreaming={isStreaming} disabled={addingDisabled} onAdd={addOutline} />
        ) : (
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex shrink-0 items-center gap-1.5 pt-0.5">
              ...
              <div className="inline-flex items-center overflow-hidden rounded-full">
                ...
                {!STUDYLOOP_EMBEDDED && !disabled && outline.type === 'pbl' && (
