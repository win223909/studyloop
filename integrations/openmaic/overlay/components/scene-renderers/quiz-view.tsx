    ...
    (async () => {
      ...
      try {
        aiResults = await Promise.all(
          shortAnswerQs.map((q) =>
            gradeShortAnswerQuestion(q, (answers[q.id] as string) ?? '', locale, controller.signal),
          ),
        );
      } catch {
        controller.abort();
  ...
  return (
    <div className="w-full h-full bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-900 overflow-hidden flex flex-col">
      {gradingError && (
        <p
          role="alert"
          className="mx-6 mt-4 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-100"
        >
          {locale.startsWith('zh')
            ? '评分暂时失败，未记录成绩。你的答案已保留，请再次提交重试。'
            : 'Grading failed; no score was recorded. Your answers are saved. Submit again to retry.'}
        </p>
      )}
