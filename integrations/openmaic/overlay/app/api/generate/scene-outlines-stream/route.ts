...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (embedded) {
      prompts.system +=
        '\nStudyLoop classroom scope: produce 4 to 6 scenes at most. Only slide, quiz and standalone-HTML interactive scenes are available. Do not use pbl, procedural-skill, image generation, video generation or external tools. Finish within six scenes and include a recap and practice.';
    }
          ...
          for (let attempt = 1; attempt <= MAX_STREAM_RETRIES + 1; attempt++) {
            try {
              ...
              for await (const chunk of textStream) {
                ...
                for (const candidate of newOutlines) {
                  // Keep the default micro-lesson within the shared request
                  // budget and restrict it to the classroom engines we ship.
                  if (embedded && parsedOutlines.length >= 6) continue;
                  const outline: SceneOutline =
                    embedded &&
                    (!['slide', 'quiz', 'interactive'].includes(candidate.type) ||
                      candidate.widgetType === 'procedural-skill')
                      ? {
                          ...candidate,
                          type: 'slide',
