...
export async function resolveModel(params: {
  ...
  // StudyLoop owns the sole provider connection. Browser state is never configuration.
  const modelString = embedded
    ? process.env.DEFAULT_MODEL
    : stageModel || params.modelString || process.env.DEFAULT_MODEL;
  if (!modelString) {
    throw new Error(
  ...
  const routed = embedded || Boolean(stageModel);
