...
// Keep the classroom under the same administrator limits as StudyLoop.
function applyStudyLoopLimits<T extends GenerateTextParams | StreamTextParams>(params: T): T {
  if (process.env.NEXT_PUBLIC_STUDYLOOP_EMBEDDED !== 'true') return params;
  const timeout = Number(process.env.STUDYLOOP_LLM_TIMEOUT_MS) || 90000;
  const outputLimit = Number(process.env.STUDYLOOP_LLM_MAX_OUTPUT_TOKENS) || 8192;
  const signals = [AbortSignal.timeout(Math.max(1000, Math.min(180000, timeout)))];
  if (params.abortSignal) signals.push(params.abortSignal);
  return {
    ...params,
    abortSignal: AbortSignal.any(signals),
    maxOutputTokens: Math.min(params.maxOutputTokens || outputLimit, outputLimit),
    maxRetries: 0,
  };
}
...
export async function callLLM<T extends GenerateTextParams>(
  ...
  params = applyStudyLoopLimits(params);
