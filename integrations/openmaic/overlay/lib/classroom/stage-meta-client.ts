...
export type StageMetaResult =
  { outcome: 'found'; meta: StageMetaView } | { outcome: 'absent' } | { outcome: 'unavailable' };
...
export async function fetchStageMeta(
  ...
  // StudyLoop classrooms live in this browser and have no cloud sidecar. Use
  // the existing local-course fallback without contacting shared persistence
  // or changing document names, generation state, ownership or deletion fences.
  if (process.env.NEXT_PUBLIC_STUDYLOOP_EMBEDDED === 'true') return { outcome: 'absent' };
