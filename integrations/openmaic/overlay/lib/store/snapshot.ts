...
async function persistSnapshot(snapshot: Omit<Snapshot, 'id'>): Promise<void> {
  const stageIds = [...new Set(snapshot.slides.map((scene) => scene.stageId))];
  for (const stageId of stageIds) assertClassroomWritable(stageId);
  const id = await db.snapshots.add(snapshot);
  if (stageIds.some(isClassroomDeleted)) {
    await db.snapshots.delete(id);
    throw new Error('A deleted classroom cannot save an undo snapshot.');
  }
}
