...
export async function loadSurvivingDocumentAssetRefs(
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_STUDYLOOP_EMBEDDED === 'true') {
      const includeStageRows = async (document: StageAssetDocument) => {
        const [mediaRows, audioRows] = await Promise.all([
          db.mediaFiles.where('stageId').equals(document.stage.id).toArray(),
          db.audioFiles.where('stageId').equals(document.stage.id).toArray(),
        ]);
        const refs = collectStageAssetRefs(document, { mediaRows, audioRows });
        for (const ref of refs.all) liveRefs.add(ref);
      };
      // A surviving classroom can have durable media allocations whose scene
      // update has not flushed yet. Those stage-owned compatibility rows are
      // live even if the authoritative document does not reference them yet.
      for (const document of documents) await includeStageRows(document);

      // The classroom library also lists unmigrated legacy stages. Their
      // global image/audio/pool refs remain live even when maic-documents has
      // never held a copy. Inspect the legacy snapshot without migrating or
      // writing it: cleanup must not create new documents or allocate assets.
      const authoritativeIds = new Set(summaries.map(({ id }) => id));
      const legacyStore = getLegacyDocumentStore();
      const legacyStages = await legacyStore.listStages();
      for (const legacyStage of legacyStages) {
        if (legacyStage.id === excludedDocumentId || authoritativeIds.has(legacyStage.id)) continue;
        const snapshot = await legacyStore.read(legacyStage.id);
        if (!snapshot || snapshot.stage.id !== legacyStage.id)
          throw new Error('A listed legacy classroom could not be inspected.');
        const { stage } = canonicalizeLegacyStage(snapshot.stage);
        const scenes = snapshot.scenes.map(canonicalizeLegacyScene);
        await includeStageRows({ stage, scenes });
      }

      // Undo snapshots can restore a surviving classroom's previous media,
      // including refs no longer present in its current document. Snapshot
      // rows can contain multiple stages; preserve each exact scene owner.
      // A malformed row makes liveness unknown, never an empty reference set.
      const snapshots = await db.snapshots.toArray();
      const snapshotScenes = new Map<string, Scene[]>();
      for (const snapshot of snapshots) {
        if (!Array.isArray(snapshot.slides))
          throw new Error('A classroom undo snapshot could not be inspected.');
        for (const rawScene of snapshot.slides) {
          if (!rawScene || typeof rawScene.stageId !== 'string' || !rawScene.stageId)
            throw new Error('An undo scene has no verifiable classroom owner.');
          if (rawScene.stageId === excludedDocumentId) continue;
          const scene = canonicalizeLegacyScene(rawScene);
          const scenes = snapshotScenes.get(scene.stageId) || [];
          scenes.push(scene);
          snapshotScenes.set(scene.stageId, scenes);
        }
      }
      for (const [stageId, scenes] of snapshotScenes) {
        const refs = collectStageAssetRefs(
          { stage: { id: stageId, name: '', createdAt: 0, updatedAt: 0 }, scenes },
          { mediaRows: [], audioRows: [] },
        );
        for (const ref of refs.document) liveRefs.add(ref);
      }
